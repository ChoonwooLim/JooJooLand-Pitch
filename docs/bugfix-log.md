...
| 2026-05-30 | 프런트엔드만 바꾼 push 가 라이브에 반영 안 됨 (화면 변화 0) | `orbitron.yaml` 이 `backend/Dockerfile`·`context: backend` 만 빌드 → 자동배포가 프런트를 재빌드하지 않음 (backend 이미지 캐시 동일 → 컨테이너만 재생성) | 루트 멀티스테이지 `Dockerfile`(node 빌드→python 백엔드에 `/app/static` COPY) 신설 + `orbitron.yaml` 을 `dockerfile: Dockerfile, context: .` 로 전환 + `main.py` 에 `spa_fallback` 서빙. frontend COPY 레이어 무효화로 vite 재빌드 자동화 | Dockerfile, orbitron.yaml, backend/app/main.py, .dockerignore |
